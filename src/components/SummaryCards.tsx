...
export function SummaryCards({ baseline, savings }: { baseline: Baseline; savings: number }) {
  const cards = [
    { label: 'Monthly income', value: formatCurrency(baseline.averageMonthlyIncome), detail: 'Average take-home income', icon: ArrowDownLeft, color: 'mint' },
    { label: 'Monthly spending', value: formatCurrency(baseline.averageMonthlyExpenses), detail: 'Average across all categories', icon: ArrowUpRight, color: 'lavender' },
  ...
  return <section className="summary-grid" aria-label="Alex’s current finances">{cards.map(({ label, value, detail, icon: Icon, color }) =>
